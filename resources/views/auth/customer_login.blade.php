...
<body class="min-h-screen bg-base-200 flex items-center justify-center p-4">
  <div class="card w-full max-w-sm bg-base-100 shadow-xl">
    <div class="card-body">
      <h2 class="card-title">เข้าสู่ระบบลูกค้า</h2>
      @if ($errors->any())
        <div class="alert alert-error text-sm">{{ $errors->first() }}</div>
      @endif
      <form method="post" action="{{ route('customer.login') }}" class="space-y-3">
        @csrf
        <input class="input input-bordered w-full" name="login" placeholder="อีเมลหรือเบอร์โทร" value="{{ old('login') }}" required>
      ...
      <div class="text-center text-sm opacity-70 mt-2">ลูกค้าใช้ดูประวัติคำสั่งซื้อ/โปรไฟล์</div>
    </div>
  </div>
</body>
</html>
