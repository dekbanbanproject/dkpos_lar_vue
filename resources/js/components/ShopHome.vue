      ...
      <div class="card bg-base-100 shadow">
        <div class="card-body">
          ...
          <ul class="menu bg-base-200 rounded-box max-h-[50vh] overflow-auto">
            <li>
              <a :class="{'active': !activeCat}" @click="setCat(null)">
                ทั้งหมด <span class="badge badge-sm ml-auto">{{ totalAll }}</span>
              </a>
            </li>
            <li v-for="c in categories" :key="c.id">
              <a :class="{'active': activeCat===c.id}" @click="setCat(c.id)">
                {{ c.name }} <span class="badge badge-sm ml-auto">{{ c.count }}</span>
              </a>
            </li>
          </ul>
        </div>
        ...
        <div v-else class="flex flex-col gap-2 max-h-[50vh] overflow-auto pr-1">
          <div v-for="(c, idx) in cart" :key="c.id" class="flex items-center justify-between gap-2 bg-base-200 rounded-xl p-2">
            ...
            <div class="flex items-center gap-1">
              ...
              <input type="number" class="input input-bordered input-xs w-14 text-center" v-model.number="c.qty" min="1" />
...
// ให้ทำงานได้ทั้งกรณีอยู่ใต้ /dkpos/public/
